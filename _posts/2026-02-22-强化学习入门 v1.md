...
\\
\text{其中, }
[f(v)]_{s} &= \max_{\pi} \sum_{a} \pi(a\mid s) q(s,a), s\in S
\end{align}
$$
这里的形式很自然让人联想到数学中的 "不动点". 针对 (简化后的) BOE: $v=f(v)$, 我们首先可证, $f(v)$ 是 contraction mapping; 然后利用 contraction mapping theorem 可得: $v^{\star}$ 的存在性以及唯一性. 此外, 我们还知道如何求解出 $v^{\star}$: 迭代求解! 并且收敛性也得到了保证.
$$
\begin{align}
