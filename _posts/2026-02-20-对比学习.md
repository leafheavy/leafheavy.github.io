...
title: "对比学习论文串烧"
date: 2026-02-20
permalink: /posts/2026/02/对比学习论文串烧/
tags:
  - 论文阅读
  - 对比学习
category: tech
...
考虑代理任务是个体判别任务, 其中正样本对 $\lbrace (\boldsymbol{v_i}, f_{i}) \rbrace_{i=1}^{n}$ 理解为: 前后时间步模型针对同一个输入得到的 feature; 负样本对 $\lbrace (\boldsymbol{v_i}, f_{j}) \rbrace_{j=1}^{m}, i \neq j, \forall i \in [1,n]$ 则表示: 针对第 $i$-th 输入 $\boldsymbol{v_i}$, 其余输入的 feature.
...
<div align="center">
<img src="/images/blogs/对比学习论文串烧/SimCLR_framework.png" alt="image-SimCLR-Framework" width="70%"/>
</div>

其给笔者比较有启发的点是: 在最后引入了一个 projection head $g(\cdot)$ (其实是一个 MLP, 后经 SimCLR v2 实验发现, 2 层效果最佳), 大幅提升了无监督预训练模型在下游任务上的表现. 并且该提升已经被后续众多工作验证. **其提升机理似乎是个有意思, 可挖掘的 topic?**

接下来, 笔者总结一下 SimCLR 是如何回答对比学习的三个本质问题的?

### 如何设计代理任务?

同样使用的是 [InstDisc](#sec:instdisc) 所提及的个体判别任务 (Instance Discrimination). 沿着对比学习的发展脉络, 笔者发现代理任务基本已经确定下来, 成为对比学习新工作的基本组件了, 较少在代理任务上进行创新. 而更多的工作将精力放在了对正负样本对的构造上.

### 如何构造正负数据对?
如图所示, 将每个输入 $x$ 进行两次数据增强所得到的 $\widetilde{x_i}, \widetilde{x_j}$ 组织成为一个正样本对; 将和其余输入所增强得到的数据均视作负样本. 

让我们来拆解一下:
1. 假设批次大小为 $N$ (原始图像数), 增强后样本数为 $2N$. 相同原始图像增强的两个样本构成 $1$ 个正样本对 $\lbrace (\widetilde{x_i}, \widetilde{x_j}) \rbrace_{k=1}^{N}$, 因此总共有 $N$ 个正样本对.
2. 以 $\widetilde{x_i}$ 为 Anchor，$\widetilde{x_j}$ 为正样本。此时负样本是剩余的 $2N-2$ 个样本。这产生了 $2(N-1)$ 个负样本对.
3. 以 $\widetilde{x_j}$ 为 Anchor，$\widetilde{x_i}$ 为正样本. 此时负样本也是剩余的 $2N-2$ 个样本. 这又产生了 $2(N-1)$ 个负样本对.

### 如何设计 Contrastive Loss?
SimCLR 提出了新的损失函数: NT-Xent (Normalized Temperature-scaled Cross Entropy Loss). **笔者认为和 [InfoNCE](#infonce_loss_construction) 没有本质区别, 仅要求: $z_i, z_j$ 是经过 $\ell_2$ 归一化的且使用余弦相似度替代原本的单纯内积计算.**
...
其中, $\operatorname{sim}$ 表示余弦相似度; $\ell_{i, j}$ 表示以 $\widetilde{x_i}$ 作为 anchor, $\widetilde{x_j}$ 为正样本来计算损失. 由于针对同一个输入, 有两个 anchor, 因此总损失函数有两部分:
...
这里之所以能够保证 $\widetilde{x_{2k-1}}, \widetilde{x_{2k}}$ 能够构成正样本对, 是因为在数据增强阶段, SimCLR 将第 $k$-th 样本增强后的索引分别映射为 $2k-1, 2k$.

> 笔者这里不过多赘述 MoCo v2 和 SimCLR v2, 由于并没有较为本质的启发性观点, 更多的是 MoCo 和 SimCLR 彼此之间的借鉴以及工程上的 trick 叠加.

<span id="sec:swav"></span>

## [SWaV](https://proceedings.mlr.press/v119/chen20j/chen20j.pdf)

<span id="sec:infomin"></span>

## [InfoMin](https://proceedings.neurips.cc/paper_files/paper/2020/file/4c2e5eaae9152079b9e95845750bb9ab-Paper.pdf)

这里的 "Min" 表示的是并不是最小化; 而是适量地最大化互信息.

<span id="sec:byol"></span>

## [BYOL](https://proceedings.neurips.cc/paper_files/paper/2020/file/f3ada80d5c4ee70142b17b8192b2958e-Paper.pdf)

负样本的作用是什么, 我们为什么不能抛弃掉负样本呢?

我们不妨考虑一下只有正样本的情况. 那么此时的损失函数则是会将所有正样本尽可能接近; 进而导致了 "模型坍缩" 的情况发生, 即: 模型对所有输入输出相同的预测输出——这正是仅有正样本的捷径解.

那么我们能仅有正样本进行特征学习吗? BYOL 给出了肯定的答案.

> BYOL 初期被社区质疑并没有完全抛弃负样本, 而是由于 BN, 隐式利用了负样本进行学习. 因此 BYOL 作者又出了[一篇工作](https://arxiv.org/pdf/2010.10241)进行对线.

<div align="center">
<img src="/images/blogs/对比学习论文串烧/BYOL_framework.png" alt="image-BYOL-Framework" width="70%"/>
</div>

其本质是在 SimCLR 上进行延申拓展.
- 新增 projection head $q_{\theta}$.
- 将代理任务设置为: 预测得到的 $q_{\theta}(z_{\theta})$ 有多接近已经停止梯度反传的 $z_{\xi}'$. 因此 BYOL 选用的损失函数则是简单的 $MSE$.

<span id="sec:simsiam"></span>
...
何凯明大佬再次出手, 尝试一统对比学习领域.

<span id="sec:clip"></span>

## [CLIP](https://proceedings.mlr.press/v119/chen20j/chen20j.pdf)
...
> ## Reference
> [1] [InstDisc 原文](https://openaccess.thecvf.com/content_cvpr_2018/papers/Wu_Unsupervised_Feature_Learning_CVPR_2018_paper.pdf)
>
> [2] [CPC 原文](https://arxiv.org/pdf/1807.03748)
>
> [3] [MoCo 论文精读](https://www.bilibili.com/video/BV1C3411s7t9?spm_id_from=333.788.videopod.sections&vd_source=a310f211c2101503fd8e3dc63176607c)
>
> [4] [MoCo 原文](https://openaccess.thecvf.com/content_CVPR_2020/papers/He_Momentum_Contrast_for_Unsupervised_Visual_Representation_Learning_CVPR_2020_paper.pdf)
>
> [5] [对比学习论文串讲](https://www.bilibili.com/video/BV19S4y1M7hm/?spm_id_from=333.337.search-card.all.click&vd_source=a310f211c2101503fd8e3dc63176607c)
>
> [6] [SimCLR 原文](https://proceedings.mlr.press/v119/chen20j/chen20j.pdf)
>
> [7] [SWaV 原文](https://proceedings.mlr.press/v119/chen20j/chen20j.pdf)
>