...
### 如何构造正负数据对?
考虑代理任务是个体判别任务, 因此正样本对是 $\lbrace \boldsymbol{v}_i, f_i \rbrace_{i=1}^{n}$, 负样本对是 $\lbrace \boldsymbol{v}_i, f_j \rbrace_{j=1}^{m}, \forall i\in [1, n]$.

| 定义 | 理解 |
| ------- | ---- |
| $\lbrace \boldsymbol{v}_i, f_i \rbrace_{i=1}^{n}$ | 前后时间步模型针对同一个输入得到的 feature |
| $\lbrace \boldsymbol{v}_i, f_j \rbrace_{j=1}^{m}, i\neq j \text{ and } \forall i\in [1, n]$ | 针对第 $i$-th 输入 $\boldsymbol{v}_i$, 其余输入的 feature |
...
由于笔者认为 CPC 的核心贡献在于 InfoNCE 的提出, 因此该篇工作首先将 InfoNCE 的来龙去脉以一家之言进行阐述分析. 最后再将 CPC 归纳进入对比学习的框架中, 即 CPC 是如何回答解决对比学习的三个本质问题. 
...
因此，CPC 不直接在数据空间 (Data Space) 去建模 $p(x|c)$ ，而是将其映射到一个紧凑的隐空间 (Latent Space), 并通过最大化未来信号 $x$ 与当前上下文 $c$ 之间的[互信息 (Mutual Information, MI)](https://zhida.zhihu.com/search/3715946346460009718), 迫使模型学到真正有用的高级语义表示.
...
其中, $x^+, x^-$ 分别表示正样本和负样本. 在最小化的过程中, $f_{opt}(x, c) \propto \frac{p(x|c)}{p(x)}$. InfoNCE 摆脱了对概率分布的依赖, 且数学层面可证: **最小化 InfoNCE 本身，就在严格推高互信息的一个下界**.
...
> 我们不难发现: 最小化 $\mathcal{L}_{InfoNCE}$ 等价于最大化 $\log(N) - \mathcal{L}_{InfoNCE}$, 从而理论层面保证推高互信息的下界.
> 
> $$
> \begin{proof}
> ##### 1. 定义数据生成过程 (概率模型)
...
- **正样本 $x_{pos} \sim p(x|c)$:** 在物理世界里, 未来的观测 $x$ 是由当前的上下文 $c$ 决定的 (或者高度相关的).
