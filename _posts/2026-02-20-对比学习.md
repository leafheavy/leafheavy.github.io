...
考虑代理任务是个体判别任务, 其中正样本对 $\lbrace \boldsymbol{v_{i}}, f_{i} \rbrace_{i=1}^{n}$理解为: 前后时间步模型针对同一个输入得到的 feature; 负样本对 $\lbrace \boldsymbol{v_{i}}, f_{j} \rbrace_{j=1}^{m}, i \neq j, \forall i \in [1,n]$ 则表示: 针对第 $i$-th 输入 ${\boldsymbol{v}}_i$, 其余输入的 feature.
