...
- 正样本对 $\left\{x_i^{\mathrm{anch}}, x_i^{\mathrm{pos}}\right\}$；负样本对：$\left\{x_i^{\mathrm{anch}}, x_j^{\mathrm{neg}}\right\},\ \forall i \neq j$.

- 将 $x_i^{\mathrm{anch}}$ 视作查询 (query): $x^{\mathrm{query}}$; 将正/负样本组织成为字典 (的 key): $\left\{x_i^{\mathrm{key}}\right\}_{i=0}^{N}$.

<!-- - 正样本对 $\left\{x^{\text{anch}}_{i}, x^{\text{pos}}_i\right\}$ 中, 负样本对 $\left\{x^{\text{anch}}_{i}, x^{\text{neg}}_j\right\}, \ \forall i \neq j$. -->

<!-- - 将 $x^{\text{anch}}_{i}$ 视作查询 (query): $x^{\text{query}}$; 将正/负样本组织成为字典 (的 key): $\left\{x_i^{\text{key}} \right\}_{i=0}^{N}$. -->
...
- **设计代理任务:** MoCo 选择[个体判别任务](https://zhida.zhihu.com/search/3715216596552261941). 该任务的核心思想是: 将每个独立个体作为一类进行分类任务.
