...
- 正样本对 $\{x^{\text{anch}}_{i}, x^{\text{pos}}_i\}$ 中, 负样本对 $\{x^{\text{anch}}_{i}, x^{\text{neg}}_j\}\, \forall i \neq j$.
- 将 $x^{\text{anch}}_{i}$ 视作查询 (query): $x^{\text{query}}$; 将正/负样本组织成为字典 (的 key): $\{x_i^{\text{key}} \}_{i=0}^{N}$.
...
- **设计 Contrastive Loss:** MoCo 基于 dictionary look-up 抽象, 指出 Contrastive Loss 的设计原则 **whose value is low when q is similar to its positive key k+ and dissimilar to all other keys (considered negative keys for q)**. 最终 MoCo 选择了 InfoNCE 损失函数, 如下所示.
