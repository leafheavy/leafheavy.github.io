...
考虑代理任务是个体判别任务, 因此正样本对是 <span>$\lbrace \boldsymbol{v}_i, f_i \rbrace_{i=1}^{n}$</span>, 负样本对是 <span>$\lbrace \boldsymbol{v}_i, f_j \rbrace_{j=1}^{m}, \forall i\in [1, n]$</span>.

- 正样本对 <span>$\lbrace \boldsymbol{v}_i, f_i \rbrace_{i=1}^{n}$</span>: 前后时间步模型针对同一个输入得到的 feature
- 负样本对 <span>$\lbrace \boldsymbol{v}_i, f_j \rbrace_{j=1}^{m}, i \neq j, \forall i \in [1,n]$</span>: 针对第 $i$-th 输入 $\boldsymbol{v}_i$, 其余输入的 feature.
