...
考虑代理任务是个体判别任务, 其中正样本对 <span>$\lbrace \mathbf{v}_{i}, f_{i} \rbrace_{i=1}^{n}$</span>理解为: 前后时间步模型针对同一个输入得到的 feature; 负样本对 <span>$\lbrace \mathbf{v}_{i}, f_{j} \rbrace_{j=1}^{m}, i \neq j, \forall i \in [1,n]$</span>则表示: 针对第 <span>$i$-th 输入 $\boldsymbol{v}_i$</span>, 其余输入的 feature.

$\lbrace \boldsymbol{v}s_i, f_i \rbrace_{i=1}^{n}$
