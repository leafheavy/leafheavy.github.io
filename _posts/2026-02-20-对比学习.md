...
### 如何构造正负数据对?
如图所示, 将每个输入 $x$ 进行两次数据增强所得到的 $\tilde{x}_i, \tilde{x}_j$ 组织成为一个正样本对; 将和其余输入所增强得到的数据均视作负样本. 

让我们来拆解一下:
1. 假设批次大小为 $N$ (原始图像数), 增强后样本数为 $2N$. 相同原始图像增强的两个样本构成 $1$ 个正样本对 $\lbrace (\tilde{x}_i, \tilde{x}_j) \rbrace_{k=1}^{N}$, 因此总共有 $N$ 个正样本对.
2. 以 $\tilde{x}_i$ 为 Anchor，$\tilde{x}_j$ 为正样本。此时负样本是剩余的 $2N-2$ 个样本。这产生了 $2(N-1)$ 个负样本对.
3. 以 $\tilde{x}_j$ 为 Anchor，$\tilde{x}_i$ 为正样本. 此时负样本也是剩余的 $2N-2$ 个样本. 这又产生了 $2(N-1)$ 个负样本对.
...
其中, $\operatorname{sim}$ 表示余弦相似度; $\ell_{i, j}$ 表示以 $\tilde{x}_i$ 作为 anchor, $\tilde{x}_j$ 为正样本来计算损失. 由于针对同一个输入, 有两个 anchor, 因此总损失函数有两部分:
...
这里之所以能够保证 $\tilde{x}_{2k-1}, \tilde{x}_{2k}$ 能够构成正样本对, 是因为在数据增强阶段, SimCLR 将第 $k$-th 样本增强后的索引分别映射为 $2k-1, 2k$.
